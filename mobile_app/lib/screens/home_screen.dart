...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(
        padding: EdgeInsets.symmetric(horizontal: screenSize.width * .025),
        child: SingleChildScrollView(
          child: Column(children: _buildPageContent(screenSize)),
        ),
      ),
    );
  }

  List<Widget> _buildPageContent(Size screenSize) =>
      _listsName.map((e) => _buildCardsList(screenSize, e)).toList();

  Column _buildCardsList(Size screenSize, String label) {
    _postersLinks.shuffle();

    List<Widget> listItems = _postersLinks
